# Testing `simply` Skill

This document outlines the evaluation and verification workflows for the `simply` explanation skill.

## 1. Automated Evaluation (`EVAL.txtpb`)

The `EVAL.txtpb` file defines a standardized verification case to evaluate model compliance with the structured explanation layout instructions. It asserts that when a user asks to simply explain a concept, the model strictly structures its output into four sections:
...
To manually verify that the `simply` skill functions correctly:
1. Prompt the assistant with:
   ```
   Simply explain the concept of click_view and how it is used in Google Ads API
   ```
